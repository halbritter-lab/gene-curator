...
async def get_gene_statistics(db: Session = Depends(get_db)) -> Any:
    """
...
async def get_genes_summary(
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of genes to return"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
) -> Any:
...
async def create_gene(
    ...
    """
    return gene_crud.create(db=db, gene_create=gene_data, user_id=str(current_user.id))


@router.post(
    "/bulk", response_model=GeneBulkCreateResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_gene(
    ...
    gene = gene_crud.update(
        db=db, gene_id=gene_id, gene_update=gene_update, user_id=str(current_user.id)
    )
    if not gene:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Gene not found"
...
async def get_gene_history(
    ...
    history = (
        db.query(ChangeLog)
        .filter(ChangeLog.entity_type == "gene", ChangeLog.entity_id == gene_id)
