...
from sqlalchemy import (
    Column, String, Boolean, DateTime, Text, Numeric, Integer, BigInteger,
    ForeignKey, ARRAY, Enum, Index
)
...
class User(Base):
    ...
    role = Column(Enum(UserRole, values_callable=lambda obj: [e.value for e in obj]), nullable=False, default="viewer", index=True)
    ...
    # Relationships
    created_genes = relationship("Gene", foreign_keys="[Gene.created_by]", back_populates="creator")
    updated_genes = relationship("Gene", foreign_keys="[Gene.updated_by]", back_populates="updater")
    created_precurations = relationship("Precuration", foreign_keys="[Precuration.created_by]", back_populates="creator")
    created_curations = relationship("Curation", foreign_keys="[Curation.created_by]", back_populates="creator")
    approved_curations = relationship("Curation", foreign_keys="[Curation.approved_by]", back_populates="approver")
...
class Gene(Base):
    ...
    # Relationships
    creator = relationship("User", foreign_keys=[created_by], back_populates="created_genes")
    updater = relationship("User", foreign_keys=[updated_by], back_populates="updated_genes")
...
class Precuration(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    gene_id = Column(UUID(as_uuid=True), ForeignKey("genes.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Core precuration fields
    mondo_id = Column(String(50), nullable=False, index=True)
    mode_of_inheritance = Column(Text, nullable=False)
    lumping_splitting_decision = Column(Enum(PrecurationDecision, values_callable=lambda obj: [e.value for e in obj]), default="Undecided", index=True)
    rationale = Column(Text)
    
    # Workflow status
    status = Column(Enum(WorkflowStatus, values_callable=lambda obj: [e.value for e in obj]), default="Draft", index=True)
    ...
    creator = relationship("User", foreign_keys=[created_by], back_populates="created_precurations")
...
class Curation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    gene_id = Column(UUID(as_uuid=True), ForeignKey("genes.id", ondelete="CASCADE"), nullable=False, index=True)
    precuration_id = Column(UUID(as_uuid=True), ForeignKey("precurations.id", ondelete="SET NULL"))
    ...
    verdict = Column(Enum(CurationVerdict, values_callable=lambda obj: [e.value for e in obj]), nullable=False, index=True)
    ...
    creator = relationship("User", foreign_keys=[created_by], back_populates="created_curations")
    approver = relationship("User", foreign_keys=[approved_by], back_populates="approved_curations")
