                ...
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" md="6">
                      ...
                      <div class="mb-4" v-if="curation.has_contradictory_evidence">
              ...
              <v-card>
                ...
                <v-card-text>
                  <v-tabs v-model="evidenceTab">
                    ...
                    <v-tab v-if="curation.details.contradictory_evidence" value="contradictory">Contradictory Evidence</v-tab>
                    <v-tab v-if="curation.details.lumping_splitting_details" value="lumping">Lumping/Splitting</v-tab>
                  </v-tabs>
                  ...
                  <v-tabs-window v-model="evidenceTab" class="mt-4">
                    ...
                    <v-tabs-window-item value="genetic">
                      <div v-if="curation.details.genetic_evidence">
                        <div v-for="(evidence, category) in curation.details.genetic_evidence" :key="category" class="mb-4">
                          <h4 class="text-h6 mb-2 text-capitalize">{{ category.replace('_', ' ') }}</h4>
                          <v-card variant="outlined" class="mb-2" v-for="(item, index) in evidence" :key="index">
                            <v-card-text>
                              <div v-if="item.pmid" class="mb-2">
                                <strong>PMID:</strong> 
                                <a :href="`https://pubmed.ncbi.nlm.nih.gov/${item.pmid}`" target="_blank" class="text-primary">
                                  {{ item.pmid }}
                                </a>
                              </div>
                    ...
                    <v-tabs-window-item value="experimental">
                      <div v-if="curation.details.experimental_evidence">
                        <div v-for="(evidence, category) in curation.details.experimental_evidence" :key="category" class="mb-4">
                          <h4 class="text-h6 mb-2 text-capitalize">{{ category.replace('_', ' ') }}</h4>
                          <v-card variant="outlined" class="mb-2" v-for="(item, index) in evidence" :key="index">
                            <v-card-text>
                              <div v-if="item.pmid" class="mb-2">
                    ...
                    <v-tabs-window-item value="contradictory">
                      <div v-if="curation.details.contradictory_evidence">
                        <v-card variant="outlined" class="mb-2" v-for="(item, index) in curation.details.contradictory_evidence" :key="index">
                          <v-card-text>
                            <div v-if="item.pmid" class="mb-2">
                              <strong>PMID:</strong> 
                              <a :href="`https://pubmed.ncbi.nlm.nih.gov/${item.pmid}`" target="_blank" class="text-primary">
                    ...
                    <v-tabs-window-item value="lumping">
                      <div v-if="curation.details.lumping_splitting_details">
                        <div class="text-body-1 whitespace-pre-wrap">{{ curation.details.lumping_splitting_details }}</div>
                      </div>
                      <div v-else class="text-center py-4 text-medium-emphasis">
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="3">
                      ...
                      <div class="text-body-1">
                        ...
                        <span v-if="!curation.contributors?.length" class="text-medium-emphasis">None</span>
                      </div>
                    </v-col>
                  </v-row>
                </v-card-text>
...
const canEdit = computed(() => {
  return authStore.isCurator && curation.value && ['Draft', 'In_Primary_Review'].includes(curation.value.status)
})
